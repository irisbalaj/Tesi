\fancyhead[LO, RE] {Capitolo 3: Studio delle Tecnologie}
\section{Stealth 3000}
La maggior parte delle aziende tessili di larga scala in Italia utilizza un \textit{\textbf{ERP\ped{g}}} specifico, Stealth 3000, sviluppato dall'azienda italiana Dedagroup.\\
...
Si tratta di un'\textit{\textbf{applet\ped{g}}} Java, accessibile solo internamente all'azienda da Internet Explorer e Firefox.\\
...
Di seguito in \textit{figura 3.1} la form di anagrafica Soggetto vista da Stealth 3000:
\begin{figure}[!h]
\thispagestyle{empty}
\centering
...
\end{figure}
\newpage
In \textit{tabella 3.1} vediamo i Dati Testata, ovvero il blocco principale utilizzato come filtro di ricerca o di dati primari:
\begin{table}[!h]
	\begin{center}
...
\end{table}
\newpage
In \textit{tabella 3.2} vediamo i Dati Generali, ovvero il blocco secondario di maggior importanza, utilizzato come dettaglio dei dati primari:
...
\end{longtable}
\newpage
\subsection{Condizioni di Vendita}
Di seguito in \textit{figura 3.2} vediamo la form delle Condizioni di Vendita (colloquialmente, Condizioni Commerciali) accessibili dalla schermata dei Soggetti in \textit{figura 3.1}.
...
Di seguito in \textit{figura 3.3} si può vedere un'anagrafica di gestione dei Modelli da Stealth 3000:
\begin{figure}[!h]
\thispagestyle{empty}
\centering
\includegraphics[scale=0.90]{img/Modello.png}
\caption{Anagrafica Modello}
\end{figure}
\newpage

In \textit{tabella 3.4} vediamo i Dati di Testata dell'anagrafica dei Modelli, ovvero il blocco principale usato come filtro di ricerca o inserimento dei dati primari:
\begin{longtable}{| c | c |}%{| p{.20\textwidth} | p{.80\textwidth} |}
...
In \textit{tabella 3.5} vediamo i Dati di Base dell'anagrafica dei Modelli, ovvero il blocco principale di dati di dettaglio:
\begin{longtable}{| c | c |}%{| p{.20\textwidth} | p{.80\textwidth} |}
...
Di seguito in \textit{figura 3.4} si può vedere una anagrafica di gestione Parti in Stealth 3000:
...
In \textit{tabella 3.6} vediamo i Dati di Testata delle Parti, ovvero il blocco principale usato come filtro di ricerca o inserimento dei dati primari:
...
In \textit{tabella 3.7} vediamo i Dati di Base dell'anagrafica delle Parti, ovvero il blocco principale di dati di dettaglio:
...
Di seguito in \textit{figura 3.5} si può vedere un'anagrafica del Modello-Parte vista da stealth 3000 ed una descrizione dei Dati:
\begin{figure}[!h]
\thispagestyle{empty}
\centering
...
In \textit{tabella 3.8} vediamo i Dati di Testata dei Modelli-Parte, ovvero il blocco principale usato come filtro di ricerca o inserimento dei dati primari:
\begin{longtable}{| c | c |}%{| p{.20\textwidth} | p{.80\textwidth} |}
...
\end{longtable}
\newpage
In \textit{tabella 3.9} vediamo i Dati di Base dell'anagrafica dei Modelli-Parte, ovvero il blocco principale di dati di dettaglio:
\begin{longtable}{| c | c |}%{| p{.20\textwidth} | p{.80\textwidth} |}
	...
	\caption{Modello-Parte: Dati base}

\end{longtable}
\subsection{Listini di Vendita}
...
Di seguito in \textit{figura 3.6} possiamo vedere un'anagrafica di gestione Stealth 3000 per i Listini di Vendita, con la descrizione dei dati principali.\\

\begin{figure}[!h]
\thispagestyle{empty}
\centering
\includegraphics[scale=0.80]{img/Listino.png}
\caption{Anagrafica Listino di Vendita}
\end{figure}
\newpage

In \textit{tabella 3.10} vediamo i Dati di Testata dei Listini di Vendita, ovvero il blocco principale usato come filtro di ricerca o inserimento dei dati primari:
\begin{longtable}{| c | c |}%{| p{.20\textwidth} | p{.80\textwidth} |}
...
\end{longtable}
\newpage
In \textit{tabella 3.11} vediamo i dati delle Righe di Listino, ovvero il blocco principale di dati di dettaglio:
\begin{longtable}{| c | c |}%{| p{.20\textwidth} | p{.80\textwidth} |}
...
\end{longtable}
\newpage
\section{Oracle PL/SQL Developer}
PL/SQL Developer è un \textbf{\textit{IDE\ped{g}}} creato per sviluppare unità di programma memorizzato in un database Oracle.\\
SQL nasce come linguaggio per interrogazioni ad un database, che siano di estrazione o modifica dati, ma non permette di manipolare i dati in maniera estensiva, caratteristica invece di un linguaggio procedurale; istruzioni condizionali (IF ELSE) e cicli di iterazione, oltre a creazione di variabili sono le fondamenta alla base di programmi ed algoritmi complessi, ed è questo il vantaggio di PL/SQL.\cite{plsql}
\subsection{Scrittura di un programma PL/SQL}
PL/SQL permette di creare script principalmente come funzioni, procedure oppure \textbf{\textit{package\ped{g}}} che le contengono. Tutte le unità di programma sono accessibili da altre funzioni all'interno dello stesso database se appartengono allo stesso utente di accesso.\\
...
\end{itemize}
\subsection{Debug di un programma}
Una funzionalità caratteristica e particolarmente utile di PL/SQL è quella di poter eseguire il \textbf{\textit{debug\ped{g}}} di una funzione o di un intero package.\\
Questa funzione permette, come la maggior parte dei compilatori nei linguaggi di programmazione, di analizzare l'esecuzione del programma passo per passo, con l'obiettivo di individuare criticità in determinati punti del codice in \textit{Run-Time}. Diventa particolarmente utile nel caso si abbia a che fare con i refcursor spiegati in precedenza, ma ovviamente la funzionalità è importante in ogni parte del programma ed è parte fondamentale della verifica del codice nella sua interezza.\\
...
\subsection{Piano di esecuzione}
...
\end{figure}
\newpage
Con riferimento alla \textit{figura 3.7} possiamo prendere in analisi alcune delle funzionalità del piano di esecizione. Le modalità di giunzione tra le tabelle vengono specificate dal tipo di indice utilizzato; i vari tipi di indice sono:
\begin{itemize}
...
Il software permette di inserire una o più query di selezione come modello di partenza \textit{figura 3.8} e mostra in real time il risultato all'interno del report costruito tramite l'editor apposito \textit{figura 3.9}
\newpage
\begin{figure}[!h]
\thispagestyle{empty}
\centering
...
\end{figure}
Viene data la possibilità di scrivere funzioni e \textit{trigger} come unità di programma in PL/SQL, richiamabili in ogni momento, il che permette di manipolare i dati estratti dalle query di modello.\cite{rep}

\section{Tecnologia per l'aggiornamento}
